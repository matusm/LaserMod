...
namespace LaserMod
{
    public class ParameterContainer
    {
        ...
        public double BeatTotal => TotalizeToHz(totFitter.Carrier);
        public double BeatDispTotal => TotalizeToHz(totFitter.CarrierDispersion);
        public double BeatStat => TotalizeToHz(movFitter.BeatFrequency);
        public double BeatDispStat => TotalizeToHz(movFitter.BeatFrequencyDispersion);
        public double BeatLSQ => TotalizeToHz(movFitter.BeatFrequencyLSQ);
        public double BeatDispLSQ => TotalizeToHz(movFitter.BeatFrequencyDispersionLSQ);
        ...
        public string ToOutputString(OutputType outputType)
        {
            switch (outputType)
            {
                ...
                case OutputType.Verbose:
                    ...
                    sb.AppendLine($"Beat frequency (tot): {BeatTotal * 1e-6:F3} ± {BeatDispTotal * 1e-6:F3} MHz");
                    sb.AppendLine($"Beat frequency (sta): {BeatStat * 1e-6:F3} ± {BeatDispStat * 1e-6:F3} MHz");
                    sb.AppendLine($"Beat frequency (LSQ): {BeatLSQ * 1e-6:F3} ± {BeatDispLSQ * 1e-6:F3} MHz");
                ...
                case OutputType.SingleLine:
                    return $"{Filename,-20} {GateTime * 1e6,5:F0} {WindowSize,6} {BeatTotal * 1e-6,6:F4} {BeatDispTotal * 1e-6,6:F4} {BeatLSQ * 1e-6,6:F4} {BeatDispLSQ * 1e-6,6:F4} {MppStat * 1e-6,6:F4} {MppDispStat * 1e-6,6:F4} {MppLSQ * 1e-6,6:F4} {MppDispLSQ * 1e-6,6:F4} {Tau * 1e6,5:F1} {TauDisp * 1e6,3:F1} {ModulationFrequency * 1e-3,6:F4} {ModulationFrequencyDisp * 1e-3,6:F4} {Mpp * 1e-6,6:F4} {MppUncert * 1e-6,6:F4}";
                case OutputType.CsvLine:
                    return $"{Filename},{GateTime * 1e6:F0},{WindowSize},{BeatTotal * 1e-6:F4},{BeatDispTotal * 1e-6:F4},{BeatLSQ * 1e-6:F4},{BeatDispLSQ * 1e-6:F4},{MppStat * 1e-6:F4},{MppDispStat * 1e-6:F4},{MppLSQ * 1e-6:F4},{MppDispLSQ * 1e-6:F4},{Tau * 1e6:F1},{TauDisp * 1e6:F1},{ModulationFrequency * 1e-3:F4},{ModulationFrequencyDisp * 1e-3:F4},{Mpp * 1e-6:F4},{MppUncert * 1e-6:F4}";
                case OutputType.CsvHeader:
                    return "filename,gate time / µs,windows size,overall beat / MHz,overall standard deviation / MHz," +
                        "beat from LSQ fit / MHz,standard deviation of beat from LSQ fit / MHz,Mpp from statistics / MHz,Mpp from statistics standard deviation / MHz," +
